 ...
 */
function ar_scripts() {
    wp_enqueue_style( 'ar-style', get_stylesheet_uri(), [], _S_VERSION );
    wp_enqueue_style( 'common_css', get_template_directory_uri() . '/css/common.css', [], _S_VERSION );
...
function myprefix_get_image() {
    if ( isset( $_GET['id'] ) ) {
        $image = wp_get_attachment_image( filter_input( INPUT_GET, 'id', FILTER_VALIDATE_INT ), 'medium', false, [ 'id' => 'myprefix-preview-image' ] );
        $data  = [
            'image' => $image,
        ];
...
function tooling_post_type() {
    $labels = [
        'name'               => 'Tolling',
        'singular_name'      => 'Tolling', // админ панель Добавить->Функцию
        'add_new'            => 'Add information about tolling',
        'add_new_item'       => 'Add new tolling', // заголовок тега <title>
        'edit_item'          => 'Edit tolling',
        'new_item'           => 'New',
        'all_items'          => 'All',
        'view_item'          => 'view',
        'search_items'       => 'Search tolling',
        'not_found'          => 'Tolling not found.',
        'not_found_in_trash' => 'In trash not found tolling',
        'menu_name'          => 'Tolling', // ссылка в меню в админке
    ];
    $args   = [
        'labels'        => $labels,
        'public'        => true, // благодаря этому некоторые параметры можно пропустить
